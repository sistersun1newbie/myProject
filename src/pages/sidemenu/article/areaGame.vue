<template>
    <div>
        ...
        <el-table :data="tableData.data"
            ...
            <el-table-column :label="$t('article.areaGame.game')"
                     align="center"
                     min-width="150">
                <template scope="scope">
            ...
            <el-table-column :label="$t('article.areaGame.act')"
                             align="center"
                             min-width="150">
                <template scope="scope">
                    <span style="margin-left: 10px">{{ scope.row.remindAct==1?($t('article.areaGame.type1')):($t('article.areaGame.type2')) }}</span>
                </template>
            </el-table-column>
            <el-table-column :label="$t('article.areaGame.distance')"
                             align="center"
                             min-width="150">
                <template scope="scope">
...
import {
	gametaskremindPages,
	gametaskremindAdd,
	gametaskremindUpdate,
	gametaskremindDelete,
//	gametaskremindDetail
} from 'api/api';
import storyform from './articleComponents/areaGameForm';
let pageData = {
	gameId:'',
	remindZhCnContent:'',
	remindJaJpContent:'',
...
export default {
    ...
    created() {
     ...
     watch: {
      searchValue(value){
        if(!value){
          gametaskremindPages(this.page).then(res => {
    ...
    methods: {
        ...
        onQuery() {
        	if(this.searchValue){
	          gametaskremindPages({...this.page,gameName:this.searchValue}).then(res => {
					this.tableData = res.data.obj
				})
	        }
        },
        ...
        commitForm() {
            ...
            if (this.dialogTyle === 'edit') {
                ...
                gametaskremindUpdate(this.pageData).then(res => {
                    this.init()
                    this.dialogVisible = false
                })
            } else if (this.dialogTyle === 'add') {
                gametaskremindAdd(this.pageData).then(res => {
                    this.init()
                    this.dialogVisible = false
                })
            }
